...
switch type
	case 'linear'
		...
		fn = @fn_linear;		
		W = 1/sqrt(info.num_in)*weight_init(info.num_out, info.num_in);
		b = 1/sqrt(info.num_in)*weight_init(info.num_out, 1);
		params.W = W;
		params.b = b;
	case 'conv'
		% Requires filter_size, filter_depth, num_filters
		fn = @fn_conv;		
		W = 1/sqrt(info.filter_size*info.filter_size*info.filter_depth)*weight_init(info.filter_size, info.filter_size, info.filter_depth, info.num_filters);
		b = 1/sqrt(info.filter_size*info.filter_size*info.filter_depth)*weight_init(info.num_filters, 1);
		params.W = W;
		params.b = b;
	case 'pool'
