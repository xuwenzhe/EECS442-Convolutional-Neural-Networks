...
for i = num_layers:-1:2
    [~, dv_output, gradI] = model.layers(i).fwd_fn(activations{i-1},...
        model.layers(i).params, model.layers(i).hyper_params, true, dv_output);
    grad(i) = {gradI};
end
[~,~,gradI] = model.layers(1).fwd_fn(input,model.layers(1).params,...
    model.layers(1).hyper_params,true,dv_output);
grad(1) = {gradI};
end