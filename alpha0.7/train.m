...
% Momentum
if isfield(params,'momentum') alpha = params.momentum;
else alpha = 0.9; end
% Batch size
...
update_params = struct('learning_rate',lr,'weight_decay',wd,'momentum',alpha);
...
for i = 1:numIters % "epoch"
    ...
    dweight = cell(numel(model.layers),1);
    for k = 1:numel(model.layers)
        dweight(k) = {struct('W',[],'b',[])};
    end
    for j = 1:num_batches % "iteration"
        ...
        [model,dweight] = update_weights(model,grad,update_params,dweight);
        y1(j) = train_loss;
        set(h1,'XData',x(y1>0))
